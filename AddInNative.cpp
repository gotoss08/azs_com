...
static wchar_t *g_PropNames[] = {L"isopen", L"port", L"baud", L"bytesize", L"parity", L"stopbit",
								 L"command", L"answer", L"answerraw", L"error", L"loging"}; // ERP Pereverzev 16.10.2021 - added AnswerRaw
static wchar_t *g_MethodNames[] = {L"open", L"close", L"send", L"sendraw", L"receive", L"receiveraw",  L"delay", L"sendiks", // erp pereverzev 16.10.2021 - added sendraw, receiveraw
...
static wchar_t *g_PropNamesRu[] = {L"открыт", L"порт", L"скорость", L"байт", L"четность", L"стопбит",
								   L"команда", L"ответ", L"ответдвоичныеданные", L"ошибка", L"логирование"}; // ERP Pereverzev 16.10.2021 - added ќтветƒвоичныеƒанные
static wchar_t *g_MethodNamesRu[] = {L"открыть", L"закрыть", L"отправить", L"отправитьдвоичныеданные", L"получить", L"получитьдвоичныеданные", L"задержка", // ERP Pereverzev 16.10.2021 - added ќтправитьƒвоичныеƒанные, ѕолучитьƒвоичныеƒанные
									 L"отправитьикс", L"в16", L"старттаймер", L"стоптаймер", L"отправить16",
									 L"получить16", L"из16", L"получитьномерверсии", L"тестустройства",
									 L"инимари€", L"отправитьмари€",
...
bool CAddInNative::GetPropVal(const long lPropNum, tVariant* pvarPropVal)
{
	...
	case ePropAnswer:
		...
		break;
	case ePropAnswerRaw: // ERP Pereverzev 16.10.2021
		string_to_param_raw(m_ans_str, pvarPropVal);
		break;
	case ePropLoging:
		TV_VT(pvarPropVal) = VTYPE_BOOL;
...
long CAddInNative::GetNParams(const long lMethodNum)
{
	...
	case eMethTest:
		return 1;
	case eMethSend:
	case eMethSendRaw: // ERP Pereverzev 16.10.2021
						  ...
						  tVariant *pvarParamDefValue)
{
	...
	case eMethFromHex:
	case eMethRecieve:
	case eMethRecieveRaw: // ERP Pereverzev 16.10.2021
	case eMethSendHex:
	...
	case eMethSendRaw: // ERP Pereverzev 16.10.2021
		{
			...
			{
				raw_cmd = (paParams)->pstrVal;
				m_cmd_str.clear();
				for (UINT32 i = 0; i < (paParams)->strLen; i++) {
					m_cmd_str.append(1, raw_cmd[i]);
				}
				// (UINT8) raw_cmd[2] // example - get raw ascii char
			}
			res = CAddInNative::SendRaw();
			TV_VT(pvarRetValue) = VTYPE_I4;
			TV_I4(pvarRetValue) = res;
			ret = true;
		}
...
int CAddInNative::SendRaw(void) // ERP Pereverzev 16.10.2021
{
	char	*OUTBUFFER;
	DWORD   bytes_written = 0;
	...
	memcpy(OUTBUFFER, m_cmd_str.c_str(), l);
...
int CAddInNative::RecieveRaw(void) // ERP Pereverzev 16.10.2021
{
	char	INBUFFER[1024];
	DWORD   bytes_read = 0;
	...
	for (int i = 0; i < bytes_read; i++) {
		&s.insert(i, sizeof(char), INBUFFER[i]);
	}
...
bool CAddInNative::string_to_param_raw(std::string str, tVariant* val) { // ERP Pereverzev 16.10.2021
	char* t1;
